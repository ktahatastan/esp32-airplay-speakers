...
Her profil kaynak ölçüm, firmware sürümü, tarih ve rollback değeriyle saklanır. Bu cümlenin karşılığı 2026-09-08'de yazıldı: aşağıdaki "Profil" bölümüne bakın. Profilin `factory_cal`'a nasıl yazıldığı — değerler dosyası, araç ve sahibin flaş adımı — en sondaki "Profili yazmak" bölümünde.
...
## Profili yazmak — değerler dosyası, araç, flaş

Yukarıdaki bölüm "yapılacak iş bir struct doldurmak" diyor; 2026-09-12 gecesine kadar o struct'ı dolduran tek yol yeniden derlemekti (`bench_provisional_chain()`, tezgâh yapısı) ve `factory_cal`'a bir profil yazan hiçbir araç yoktu. Sahibin isteği — crossover bu değerlerle hazır olsun, sonraki oturum bir oynatma testiyle bitsin — bu boşluğu kapatmayı gerektirdi. Üç parça, ve her birinin durduğu yer:

**Değerler dosyası: `docs/assets/measurements/drivers/profile-2026-09-12-provisional.json`.** Kayıttaki sayıların **tek** yeri. İki üyesi var: `profile`, cihaza yazılan alanlar — `hk_profile_t`'nin her alanı adıyla — ve `provenance`, on dokuz sayının her biri için durum ve kaynak: `measured` (ölçüldü; hangi kayıt), `derived` (ölçümden hesapla türetildi; hangi hesap), `placeholder` (yer tutucu — ölçülmüş bir aralığın içinde verilmiş bir karar da buraya girer, 3500 Hz gibi; hangi kapı doldurur). Provenance cihaza yazılmaz, kayıt içindir; araç eksikse dosyayı reddeder. Bu şart, çünkü aynı sayı yarın ölçülmüş olandan ayırt edilemez ve ayırt eden şey dosyanın kendisidir: bir yer tutucu, durumu yanına yazılmadan ölçüme terfi edemez. Bugünkü içeriği tezgâh yapısının derlediğiyle aynıdır: iki DC direnç ölçülmüş (4,0 / 3,5 Ω), gerisi tahmin ve yer tutucu; hangi alanın neyi beklediği [[../02-hardware/driver-measurements#11. Sonuç|sürücü ölçüm kaydı §11]]'de. Adı `provisional` taşır ve `source` alanı da öyle: profil çalışırken kendini bu adla tanıtır (aşağıda). `G0`/`G1`/`G2`/`C3` bir sayı ürettiğinde değişen yer bu dosyadır, kod değil. Tezgâh yapısındaki derlenmiş profil `factory_cal` boşken devreye giren yedek olarak kalır ve dosyayla aynı sayıları taşımak zorundadır; ikisi ayrışırsa doğru olan dosyadır.

**Araç: `firmware/tools/write_profile.py`.** Dosyayı okur, şema 2 blob'unu (116 bayt, little-endian, dolgusuz — `hk_profile_t`'nin tel biçimi) üretir ve firmware'in reddedeceği şeyi aynı adla önceden reddeder: `hk_profile_valid()`'in kuralları araçta aynen vardır — kaynak boş ya da tarih sıfır (`source`), DC direnç pozitif değil (`measurement`), subsonic ya da crossover pozitif değil ya da subsonic crossover'ın altında değil (`frequency`), kazanç ya da tavan (0, 1] dışında (`gain`, `ceiling`), referans besleme 8–26 V dışında (`reference`), bir dalın release'i sıfır (`timing`), gecikme 64 örneğin üstünde ya da iki dalda birden (`delay`), polarite 0/1 dışında (`polarity`), bütçe (0, 2] dışında ya da pencere sıfır (`budget`), kazanç strap'i 0/20/26/32/36 dışında (`amp-gain`). Reddedilecek bir blob için önce araç konuşur, kart değil. `--device-dir` ile verilen dizindeki, kartın **kendi** `factory_cal.csv`'sine — `provision_credentials.py`'nin o kart için yazdığı, `cal` ad alanı ve `schema` satırını taşıyan dosyaya — blob'u `profile` satırı olarak ekler ve `provision_credentials.py`'nin kendi `build_image()`'ıyla `factory_cal.bin`'i (`0xd000`) yeniden üretir; `schema` satırı olmayan bir dizini reddeder. ADR-0023 öncesi kartların dizinlerindeki eski kimlik satırları varsa **olduğu gibi kalır**: profil onların yanına yazılır, yerlerine değil (firmware onları artık okumaz). Ve hiçbir şeyi **flaşlamaz**.

**Flaş: sahibinin işi.** Araç `esptool … write_flash 0x13000 …` komutunu basar (`factory_cal`, boyut `firmware/partitions.csv`'den), yanına bölümü geri okuyup `--dump` ile tablo hâlinde gösteren ikinci komutu koyar ve orada durur. Sebebi sorumluluktur: yazılmış bir profil ürünün ses yolunu **açar**. `hk_main` açılışta blob'u `hk_profile_load()` ile yargılar, `ok` bulursa `hk_storage` ses iznini verir, alıcı başlar ve akış gelince `hk_audio_hw` DAC'ı canlı amfilere bırakır — tezgâh istisnası olmadan, ürün yapısında. Bu yüzden profil yalnız kademeli çift (tek amfi, bir woofer, `C_SAFE`'li bir tweeter) tezgâhta hazırken yazılır; sekiz sürücüye bağlı bir karta yazılmaz ([[../06-testing/bench-measurement-order#E — Oynatma testi (ürün yolu, sonraki oturum)|tezgâh sırası §E]]). İkinci sebep aynı bölümün öteki sakinleri: kısa ya da yanlış bir imaj `0x13000`'de kimlik bilgilerini siler ve yerine hiçbir şey koymaz (`firmware/README.md`); imajın tam `0xd000` bayt olduğuna flaşlayan bakar.

**Açılış raporu ne gösterir.** `hk_main`'in yargı satırı ve gate satırı:
...
`ok`, `hk_profile_verdict_name()`'in `HK_PROFILE_OK` için verdiği sözcüktür ve `present:ok` ses iznini tek başına veren tek gate durumudur. Aradaki `PROVISIONAL` uyarısı kaynak adı `provisional` ile başladığında basılır: ürün yapısında yer tutucuyu ilan eden şey budur (ADR-0022'nin 2026-09-13 eki); ölçülmüş bir profilde bu satır yoktur. Reddedilen bir blob `profile     present and REFUSED: <ad>` satırıyla ve `audio       profile present:<ad> · verdict muted` ile gelir; `<ad>` yukarıdaki listenin tek sözcüklük adıdır (`schema`, `source`, `measurement`, `frequency`, `gain`, `ceiling`, `reference`, `timing`, `unbuildable`, `delay`, `polarity`, `budget`, `amp-gain`). Arka uç başlarken profilin kimliğini basar: `calibration <yyyymmdd> from '<source>': crossover 3500 Hz, subsonic 55 Hz, …` — ve tezgâh yapısında bile `BENCH PROFILE IN USE AND IT WAS NOT MEASURED` uyarısı artık gelmez, çünkü saklanan profil derlenmiş olanın önüne geçer. Profilin ölçülmediğini söyleyen tek şey o satırdaki `source` dizesidir; dosyanın adının `provisional` taşımasının sebebi budur. `24000 mV` yapının beyanıdır (`CONFIG_HK_SUPPLY_MV`), kaynağın ölçümü değil; profilin referansı 12000 olduğu için iki tavan yarıya iner — sessize doğru hata, yukarıdaki tek yön.

**Ne değişmedi.** Profilin yazılabilir olması sayıların ölçüldüğü anlamına gelmez. F3'ün "hiçbir köşe ya da tavan `G0` kapanmadan `factory_cal`'a yazılmaz" kuralı bugün daraltılarak durur: yasak, bir yer tutucuyu **kalibrasyon diye** yazmaktır; kaynağı `provisional` olan ve kademeli çiftin dinleme testi için sahibin eliyle flaşlanan bir profil o yasağın içinde değildir, ve ikisini ayıran şey raporda okunan `source` dizesidir ([[../03-firmware/firmware-plan#F3 — DSP koruma zinciri|F3]]). `C3` okunmadan 24 V'ta sürücüde dinleme yoktur; okunan kazanç dosyanın `amp_gain_db` alanına girer ve profil onunla yeniden yazılır. `G0`/`G2` geldiğinde dosya değişir, blob yeniden yazılır, `source` o ölçümün adını alır — ve ancak o zaman bu bölümün adı "profili yazmak"tan "kalibrasyonu yazmak"a döner.
