...
| Özel yarısı | `release` **ortam** secret'ı `HK_SIGNING_KEY`; yerel kopya `~/.merzarkabul/hk_signing_key.pem` (`0400`) |
...
```bash
espsecure.py generate_signing_key --version 2 --scheme rsa3072 ~/.merzarkabul/hk_signing_key.pem
chmod 400 ~/.merzarkabul/hk_signing_key.pem

espsecure.py extract_public_key --version 2 \
  --keyfile ~/.merzarkabul/hk_signing_key.pem firmware/certs/hk-signing-key.pub.bin

gh api -X PUT repos/ktahatastan/esp32-airplay-speakers/environments/release
gh secret set HK_SIGNING_KEY --env release \
  --repo ktahatastan/esp32-airplay-speakers < ~/.merzarkabul/hk_signing_key.pem
```
