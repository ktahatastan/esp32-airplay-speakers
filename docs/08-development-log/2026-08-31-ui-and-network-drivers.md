...
[[../07-decisions/ADR-0005-dual-provisioning|ADR-0005]] "birlikte sunulur" diyor. Bu haliyle uygulanamıyor. Kararı tek başıma vermedim: ADR'ye dört seçenek (yalnız SoftAP / yalnız BLE / sırayla / özel protocomm katmanı) ve gerekçeleriyle açık soru olarak yazıldı. Firmware scheme'i parametre yapıyor ve **SoftAP'ı varsayılan** kullanıyor, çünkü uygulamasız yol her kullanıcı için çalışır.
...
| `idf.py -C firmware build` | **PASS** — 0 uyarı; imaj 922.208 bayt, slotun **%87'si boş** |
| **Donanımda çalıştırma** | **YAPILMADI** |

Wi-Fi, mDNS, provisioning ve mbedtls girince imaj 246 KB'dan 922 KB'a çıktı. Bütçe rahat: AirPlay yığını için ölçülen 1,46 MB ile birlikte bile bir slotun yarısından azı kullanılıyor.
