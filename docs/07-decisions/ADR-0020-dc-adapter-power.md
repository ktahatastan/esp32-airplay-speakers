...
- **Limiter tavanı adaptör bütçesinden türetilir.** Sekiz BTL kanal 70 W'ın çok üstünü çekebilir; tavan, dört amfi birlikte sürülürken 2,9 A'yı aşmayacak biçimde belirlenir ve `G1`'de tam yükte `VIN` çökmesiyle doğrulanır. Çöken `VIN` ESP32-S3'ü şarkı ortasında sıfırlar; bu bir ses kalitesi sorunu değil, bir kesinti sorunudur.
...
- Nominal besleme firmware'de bir Kconfig sabitidir: `CONFIG_HK_SUPPLY_MV`, varsayılan `24000`, aralık `8000-26000`, `firmware/main/Kconfig.projbuild` içinde. Ses profilinin tavan ölçekleme alanı bu değere göre çalışır: tezgâh referansı `HK_BENCH_REFERENCE_SUPPLY_MV = 12000`'dir, çünkü tavanlar 12 V'ta alındı; 24 V'ta tavanın aşağı ölçeklenmesi koruyucu yöndür.
...
- Adaptörün akım değeri sabittir (2,9 A); ondan türeyen şey limiter tavanıdır, tersi değil.
