...
| `release` ortamında zorunlu inceleyici yok; etiket push'u insan onayı olmadan yayımlar | Orta | Kesin | Ortam oluşturuldu ve `HK_SIGNING_KEY` **ortam** secret'ı olarak saklandı; depo secret'ı sayısı 0, yani `build` işi anahtarı okuyamıyor ve bölünme gerçek. Eksik olan yalnız onay kapısı: private depoda koruma kuralları ücretli plan istiyor, API `HTTP 422` veriyor. Bilinçli takas; yayın yine de yanmış/eşleşmeyen anahtarı reddeden üç denetimden geçiyor | orchestrator |
...
| Private depo, cihazın sürüm indirmesini imkânsız kıldı | Kritik | Kesin | Ölçüldü 2026-09-02: public depoda `releases/latest` **HTTP 200**, bizim private depomuzda tokensiz **HTTP 404** (hem indirme yolu hem API). ADR-0008 varlıkların kimlik doğrulamasız çekilmesini varsayıyor ve OTA planı cihazların token taşımamasını şart koşuyor, yani firmware'e token gömmek yasak. Karar bekliyor: ayrı bir **public sürüm deposu** (kaynak private kalır, özgünlüğü imza sağlar), ya da deponun yeniden public yapılması — private olmasının asıl sebebi anahtarın depoda olmasıydı ve o sebep artık yok. Seçilene kadar cihaz güncellenemez | orchestrator |
