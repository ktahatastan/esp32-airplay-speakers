...
- Supply budget: the DSP's supply-budget stage (the profile's `supply_budget_sq` / `supply_window_ms`, an average over both branches summed) is set from the 2.9 A adapter budget in G1 S7 with all four amplifiers driven into 4 ohm-class dummy loads and verified by VIN sag: eight BTL channels can draw far more than 70 W, and a sagging VIN resets the ESP32-S3 mid-song (ADR-0020, ADR-0022). The peak ceilings are G2's driver-protection numbers and stay beneath that budget; they do not carry it.
