...
hk_profile_verdict_t hk_profile_valid(const hk_profile_t *profile)
{
    ...
    if (!isfinite(profile->reference_pack_mv) ||
        profile->reference_pack_mv < HK_PROFILE_PACK_MV_MIN ||
        profile->reference_pack_mv > HK_PROFILE_PACK_MV_MAX) {
...
{
    if (!unit_range(ceiling_at_reference) || !positive(reference_mv) || !positive(pack_mv)) {
        return 0.0f;
    }

    const float scaled = ceiling_at_reference * (reference_mv / pack_mv);
    if (!isfinite(scaled)) {
        return 0.0f;
    }
     ...
     * safety decision. The unsafe direction -- a pack ABOVE the reference --
                                      ...
                                      hk_profile_chain_t *out)
{
    ...
    if (!positive(fs_hz) || !positive(pack_mv)) {
        return HK_PROFILE_UNBUILDABLE;
    }
    ...
    const float woofer_ceiling = hk_profile_ceiling_at(profile->woofer_ceiling,
                                                       profile->reference_pack_mv, pack_mv);
