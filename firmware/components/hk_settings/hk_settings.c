...
const hk_setting_def_t hk_settings_table[] = {
     ...
     * in, so the device can be put on the canary channel to try a release
     * before it is promoted (ADR-0008) without building it a different image —
     * a device running a different image would not be testing the release. */
